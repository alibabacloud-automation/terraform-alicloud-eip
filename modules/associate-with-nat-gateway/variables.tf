...
variable "description" {
  ...
  type        = string
  default     = "An EIP associated with NAT instance."
}
...
variable "name_regex" {
  description = "Query NAT instance based on name_regex."
  type        = string
  default     = ""
}
...
variable "computed_instances" {
  description = "List of NAT instances created by calling alicloud_nat_gateway."
  type = list(object({
    instance_type = string
    instance_ids  = list(string)
    private_ips   = list(string)
